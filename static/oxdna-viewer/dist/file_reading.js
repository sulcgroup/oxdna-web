...
function dat_chunker(datFile, current_chunk, chunk_size) {
...
function extract_next_conf() {
    ...
    if (conf_end.line_id != current_chunk_length) {
...
p_hanging_line, c_hanging_line, n_hanging_line, dat_reader = new FileReader(), nextReader = new FileReader(), previous_reader = new FileReader(), //previous and previous_previous are basicaly the same...
previousPreviousReader = new FileReader(), conf_begin = new marker, conf_end = new marker, conf_len, conf_num = 0, dat_fileout = "", datFile, //currently var so only 1 datFile stored for all systems w/ last uploaded system's dat
...
target.addEventListener("drop", function (event) {
    console.log("DROP OCCC!");
    console.log("DROP EVENT OCCURRED!", event, event.dataTransfer.files, event.dataTransfer);
    ...
    console.log("Here were the files dropped in:", files);
    ...
    if (top_file) {
        ...
        if (datFile) {
            ...
            previousPreviousReader.onload = () => {
                previous_previous_chunk = previousPreviousReader.result;
...
function readDat(num_nuc, dat_reader, system) {
    ...
    for (let i = 0; i < num_nuc; i++) {
        ...
        var current_nucleotide = elements[i + system.globalStartId];
        ...
        if ((current_nucleotide.neighbor5 == undefined || current_nucleotide.neighbor5 == null) || (current_nucleotide.neighbor5.lid < current_nucleotide.lid)) {
...
function readJson(system, json_reader) {
    ...
    for (var key in data) {
        if (data[key].length == system.systemLength()) {
...
function getNewConfig(mode) {
    ...
    for (let i = 0; i < systems.length; i++) {
        let system = systems[i];
        let num_nuc = system.systemLength(); //gets # of nuc in system
        let lines;
        if (mode == 1) {
